...
def db_worker():
    ...
    cursor = conn.cursor()
    cursor.execute('CREATE TABLE IF NOT EXISTS chat_logs (channel_id TEXT, author TEXT, message TEXT, timestamp INTEGER)')
    conn.commit()
    while True:
        channel_id, messages = log_queue.get()
        if messages:
            cursor.executemany("INSERT INTO chat_logs VALUES (?, ?, ?, ?)", 
                               [(channel_id, m["author"], m["message"], m["timestamp"]) for m in messages])
            conn.commit()
        log_queue.task_done()
...
def get_channel_state(channel_id):
    with state_lock:
        if channel_id not in channels:
            channels[channel_id] = {"chat": None, "lock": threading.Lock(), "buffer": [], "last_search": 0}
        return channels[channel_id]

def get_live_id(channel_id):
    if STATIC_VIDEO_ID: return STATIC_VIDEO_ID
    if not YOUTUBE_API_KEY: return None
    try:
        r = requests.get("https://www.googleapis.com/youtube/v3/search", params={
            "key": YOUTUBE_API_KEY, "channelId": channel_id, "eventType": "live", "type": "video", "part": "id"
        }, timeout=10)
...
def chat():
    ...
    with st["lock"]:
        # Reconnect if dead or never started
        ...
        if items:
            ...
            logger.debug("Captured %d messages", len(new_msgs))
